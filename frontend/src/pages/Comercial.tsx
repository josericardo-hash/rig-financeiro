...
export default function Comercial() {
  ...
  return (
    <div className="page">
      ...
      {tab === 'geral' && <><div className="metrics four"><MetricCard title="Receita" value={formatCurrency(1045031)} /><MetricCard title="Pedidos" value="128" /><MetricCard title="Ticket Medio" value={formatCurrency(8164)} /><MetricCard title="Margem" value="22,8%" tone="green" /></div><div className="grid two"><ChartCard title="Evolucao Mensal"><ResponsiveContainer height={300}><BarChart data={b2bB2cMensal}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="mes" /><YAxis /><Tooltip formatter={(v) => formatCurrency(Number(v))} /><Legend /><Bar dataKey="b2b" fill="#1d4ed8" /><Bar dataKey="b2c" fill="#10b981" /></BarChart></ResponsiveContainer></ChartCard><ChartCard title="B2B vs B2C"><ResponsiveContainer height={300}><PieChart><Pie data={[{ n: 'B2B', v: 80 }, { n: 'B2C', v: 20 }]} dataKey="v" nameKey="n" outerRadius={100}><Cell fill="#1d4ed8" /><Cell fill="#10b981" /></Pie><Tooltip /></PieChart></ResponsiveContainer></ChartCard></div><div className="metrics three"><MetricCard title="Melhor Mes" value="Fev/2026" detail="R$ 235K +5,6%" /><MetricCard title="Mes Atual vs Anterior" value="+8,2%" tone="green" /><MetricCard title="Media Mensal" value="R$ 208K" tone="slate" /></div></>}
