...
export default function Forecast() {
  const [metodo, setMetodo] = useState('Media Movel');
  return (
    <div className="page">
      <header><h1>Forecast - Proximos 3 Meses</h1><p>Baseado em medias historicas + sazonalidade</p><div className="segmented">{['Media Simples','Media Movel','Tendencia Linear'].map((m) => <button className={metodo === m ? 'active' : ''} onClick={() => setMetodo(m)} key={m}>{m}</button>)}<button>Recalcular</button></div></header>
